...
| `census_query_data` | Query a Census dataset for one or more variables at a specific geography. Requires FIPS codes for the target geography — use `census_resolve_geography` first to convert place names. Returns labeled estimates with margin-of-error columns alongside each estimate. A `"*"` target returns up to `limit` rows in GEOID order, paged by `offset`. | `variables[]`, `geography_level`, `geography_fips`, `parent_fips`, `county_fips`, `predicates`, `dataset`, `year`, `limit`, `offset` | `readOnlyHint: true`, `openWorldHint: false` | `dataset_not_found` (NotFound), `missing_api_key` (Unauthorized), `year_not_available` (ValidationError), `variable_not_found` (NotFound), `variables_unavailable` (ServiceUnavailable, retryable), `geography_not_supported` (ValidationError), `parent_required` (ValidationError), `parent_not_accepted` (ValidationError), `no_data` (NotFound), `too_many_variables` (ValidationError), `predicate_not_supported` (ValidationError), `upstream_error` (ServiceUnavailable, retryable) |
...
`offset` is a slice of the one upstream response the handler already fetches — no upstream cursor, no cache. A cap without it would strand rows that are reachable today: Los Angeles County has 2,498 tracts and King County 1,545 block groups, and neither has a narrower parent this tool can express. `census_compare_geographies` reaches only the ends of a sorted list, so it is no substitute. `totalRows` keeps meaning rows returned; `totalCount` is rows matched and `truncated` says rows were left out. When they were, the notice gives the range returned, says the order is GEOID order and not a ranking or a sample, and names the next `offset`. An `offset` at or past the end returns no rows with `totalCount` and a notice, not `no_data`, since the data exists. `format()` renders only the rows it is given, so the total is carried in the enrichment trailer rather than the heading.
