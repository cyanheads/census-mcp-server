...
export const censusListPredicateValues = tool('census_list_predicate_values', {
  ...
  enrichment: {
    ...
    source: z
      .string()
      .describe(
        'Where the codes came from: "dataset_dictionary" for a published value map, "live_query" for a wildcard group-by against the data endpoint.',
      ),
    notice: z
      .string()
      .optional()
      .describe(
        'Guidance when the list was truncated, when query matched no code, or when the codes returned are complete only for a named industry rather than for the dimension as a whole.',
      ),
  },
  ...
  async handler(input, ctx) {
    ...
    /** The dataset's own industry dimension, when it has one — set on the live route only. */
    let naicsCode: string | undefined;

    if (variable.values) {
      all = Object.entries(variable.values)
        .map(([code, label]) => ({ code, label }))
        .sort((a, b) => a.code.localeCompare(b.code));
      source = 'dataset_dictionary';
    } else {
      const dimensions = await variableCacheService.getFilterDimensions(dataset, year, ctx);
      naicsCode = dimensions.find((d) => NAICS_DIMENSION.test(d.code))?.code;
