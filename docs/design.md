...
**Output:** Array of `{ variable_code, label, concept, predicateType }` sorted by relevance. `variable_code` is the value to pass to `census_query_data`. The enrichment block carries `totalMatches`, the number of variables matched before the limit; when results are truncated, narrow the query to see more specific results.
