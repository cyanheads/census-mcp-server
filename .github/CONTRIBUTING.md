...
Thanks for using `census-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

- [Report a bug](https://github.com/cyanheads/census-mcp-server/issues/new?template=bug_report.yml)
- [Request a feature](https://github.com/cyanheads/census-mcp-server/issues/new?template=feature_request.yml)
...
`census-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:

- **This repo** — a tool returns wrong data, a call to the Census Data API, TIGERweb, or the Census Geocoder fails, a schema doesn't match reality, a description misleads the model.
...
3. **Redact anything sensitive.** Issues are public and permanent — no API keys, tokens, auth headers, or internal URLs in code, logs, or stack traces. `CENSUS_API_KEY` shows up in request URLs, so scrub those before pasting.

## What makes an issue actionable

- Server version, `mcp-ts-core` version, runtime (Bun / Node / Workers), and transport (stdio / HTTP).
- The dataset and vintage year involved (e.g. `acs/acs5`, 2023), and the geography level.
